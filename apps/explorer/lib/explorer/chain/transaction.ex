defmodule Explorer.Chain.Transaction do
  ...
  **Note: that `29` and `30` are exceedingly rarely, and will in practice only ever be seen in specifically generated
  ...
  def decoded_input_data(%__MODULE__{to_address: %{smart_contract: nil}, input: %{bytes: <<method_id :: binary-size(4), _ :: binary>> = data}, hash: hash}) do
    candidates_query = from(
  ...
  def decoded_input_data(%__MODULE__{to_address: %{smart_contract: nil}, input: input}) do
